...
import { Event, IncomeEvent } from "@/types/event";
...
export const renderIncomeEventDetails = (
  event: Event, 
  index: number, 
  canEdit: boolean, 
  handlers: EventHandlers
) => {
  if (event.eventType.type !== "income") return null;
  
  // Function to render distribution inputs for expectedAnnualChange
  const renderDistributionInputs = () => {
    const distribution = (event.eventType as IncomeEvent).expectedAnnualChange;
    ...
    return (
      <div className="space-y-4 pl-6 border-l-2 border-gray-200 ml-2 mt-2">
        <RadioGroup
          ...
          onValueChange={(newType) => handlers.handleIncomeDistributionTypeChange(newType as DistributionType, index)}
          disabled={!canEdit}
        ...
        {distribution.type === "fixed" && (
          <div className="grid w-full max-w-sm items-center gap-1.5">
            ...
            <Input
              ...
              onChange={(e) => handlers.handleIncomeDistributionValueChange("value", e.target.value, index)}
              disabled={!canEdit}
              step="0.1"
            />
          </div>
          ...
          <div className="flex space-x-4">
            <div className="grid w-full items-center gap-1.5">
              ...
              <Input
                ...
                onChange={(e) => handlers.handleIncomeDistributionValueChange("mean", e.target.value, index)}
                disabled={!canEdit}
                step="0.1"
              />
            </div>
            <div className="grid w-full items-center gap-1.5">
              ...
              <Input
                ...
                onChange={(e) => handlers.handleIncomeDistributionValueChange("stdDev", e.target.value, index)}
                disabled={!canEdit}
                min="0"
                step="0.1"
              />
          ...
          <div className="flex space-x-4">
            <div className="grid w-full items-center gap-1.5">
              ...
              <Input
                ...
                onChange={(e) => handlers.handleIncomeDistributionValueChange("min", e.target.value, index)}
                disabled={!canEdit}
                step="0.1"
              />
            </div>
            <div className="grid w-full items-center gap-1.5">
              ...
              <Input
                ...
                onChange={(e) => handlers.handleIncomeDistributionValueChange("max", e.target.value, index)}
                disabled={!canEdit}
                step="0.1"
              />
            </div>
  ...
  return (
    <div className="space-y-4 mt-4 border-t pt-4">
      <h3 className="font-semibold text-lg text-purple-400">Income Details</h3>
      ...
      <div className="grid w-full max-w-md items-center gap-1.5">
        ...
        <Input
          ...
          onChange={(e) => handlers.handleIncomeChange("amount", parseFloat(e.target.value) || 0, index)}
          disabled={!canEdit}
          step="100"
        />
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          <Checkbox
            id={`ss-${index}`}
            checked={event.eventType.socialSecurity}
            onCheckedChange={(checked) => handlers.handleCheckboxChange(checked, "socialSecurity", index)}
            className="border-purple-400 data-[state=checked]:bg-purple-600"
            disabled={!canEdit}
          />
          <Label htmlFor={`ss-${index}`}>Social Security</Label>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          <Checkbox
            id={`wage-${index}`}
            checked={event.eventType.wage}
            onCheckedChange={(checked) => handlers.handleCheckboxChange(checked, "wage", index)}
            className="border-purple-400 data-[state=checked]:bg-purple-600"
            disabled={!canEdit}
          />
          <Label htmlFor={`wage-${index}`}>Wage Income</Label>
        </div>
      </div>
